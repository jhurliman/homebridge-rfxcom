...
RFXComPlatform.prototype.addRFYRemoteSwitch = function(remote, device, type) {
  ...
  const uuid = UUIDGen.generate(seed)
  ...
  accessory
    ...
    .on('set', (value, callback) => {
      // Issue a stop if any switch is toggled off
      if (!value) {
      ...
      // Show the switch as on
      this.setSwitch(accessory, true)
...
RFXComPlatform.prototype.setSwitch = function(accessory, isOn) {
  this.log(`Setting switch ${accessory.context.switchID} to ${isOn}`)
...
RFXComPlatform.prototype.identify = function(acc, paired, callback) {
  this.log(`${acc.name} identify requested!`)
  callback()
}
